...
@section('content')
    <div class="container-fluid px-3 px-md-4 pt-2 pb-4">
        ...
        <x-breadcrumb title="Statistik Koleksi E-Book" icon="fas fa-book-open">
            ...
            <div class="col-12">
                <div class="card unified-card border-0 shadow-sm filter-card mb-4">
                    ...
                    <div class="card-body px-4 pb-4 pt-3">
                        <form method="GET" action="{{ route('koleksi.ebook') }}" class="row g-3 align-items-end"
                            id="filterFormEbook">
                            ...
                            <div class="col-12 col-md-3">
                                <label for="tahun" class="form-label small text-muted text-uppercase">Tahun Terbit</label>
            ...
            <div class="col-sm-6">
                <div class="card unified-card border-0 shadow-sm hover-lift h-100">
                    <div class="card-body d-flex align-items-center p-3">
                        ...
                        <div>
                            <div class="text-muted small fw-bold text-uppercase">Total Judul Ebook</div>
            ...
            @if ($prodi && $prodi !== 'initial' && $dataExists)
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    ...
                    <button type="submit" form="filterFormEbook" name="export_csv" value="1"
                        class="btn btn-success  fw-bold shadow-sm px-3"><i class="fas fa-file-csv me-2"></i> Export CSV</button>
                </div>
            @endif
            <div class="card-body">
                <div class="mb-3">
                ...
                @if ($prodi && $prodi !== 'initial' && $data->isNotEmpty())
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0 unified-table" id="myTableEbook">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Judul</th>
                            ...
                            <tbody>
                                @foreach ($data as $row)
                                    <tr>
                                        ...
                                        <td>{{ $row->Penerbit }}</td>
                                        <td>{{ $row->TahunTerbit ?? $row->Tahun_Terbit ?? '-' }}</td>
                                        <td>{{ $row->Eksemplar }}</td>
                                        <td>
